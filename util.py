...
from django.utils.feedgenerator import Atom1Feed as Feed
from fcntl import flock, LOCK_EX

class StaticFeed:
    """
    Example usage:
    class MyFeed(StaticFeed):
        def item_title(self, item):
            return item.my_title
    my_feed = MyFeed(title = "My title", link = "/", description = "My Feed")
    # somewhere in signal handler:
    my_feed.save(queryset, filename)
    """
    ...
    def save(self, queryset, filename):
        feed = Feed(**self.kwargs)
        ...
        feedfile = open(filename, 'w')
        flock(feedfile, LOCK_EX)
        feed.write(feedfile, 'utf-8')
        feedfile.close()
